...
#Region "Subs and Functions"
        ...
        Public Function flagOverdueRFIs() As String
            Dim sql As String = "Select RFIID from RFIs Where RequiredBy <= '" & DateTime.Today & "' AND Status = 'Pending'"

            Dim tbl As DataTable = db.ExecuteDataTable(sql)
            'Dim count As Integer
            Dim idString As String = ""
            Dim desc As String = ""
            Dim refNum As DataTable = Nothing

            For Each row As DataRow In tbl.Rows
                sql = "SELECT STATUS from flags where RFIID = " & row("RFIID") & " AND Status = 'Open'"
                Dim flag As String = db.ExecuteScalar(sql)
                If flag IsNot Nothing Then
                Else 'Create Flag
                    sql = "Select RefNumber From RFIs where RFIID = " & row("RFIID")
                    refNum = db.ExecuteDataTable(sql)

                    RFIID = row("RFIID")
                    desc = "RFIID : # " & refNum.Rows.Item(0).Item("RefNumber") & " - RFI RequiredBy date is past due"

                    sql = "INSERT INTO Flags "
                    sql &= "(FlagDescription,LastUpdateOn,LastUpdateBy,CreatedOn,CreatedBy,Status,RFIID) "
                    sql &= "VALUES('" & desc & "',"
                    sql &= "'" & Now() & "', "
        ...
        Public Function GetAllOpenFlags() As DataTable
            ...
            sql &= "SELECT * FROM qry_GetAllFlaggedProjects WHERE DistrictID = " & DistrictID & " UNION ALL "
            sql &= "SELECT * FROM qry_GetAllFlaggedRFIs WHERE DistrictID = " & DistrictID & "  "
        ...
        Public Sub GetFlagForEdit()
            ...
            Select Case ParentRecType
                ...
                Case "RFI"
                    sql = sql & "WHERE RFIID = " & ParentRecID
        ...
        Public Sub ResolveFlag(ByVal flagid As Integer, ByVal FlagDescription As String)
            ...
            Select Case ParentRecType
                Case "Project", "RFI"     'put in project notes
                    sql &= "ProjectID)"
