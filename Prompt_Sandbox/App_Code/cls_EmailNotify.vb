...
#Region "Subs and Functions"
        ...
        Private Function GetNotifyEmailAddresses(ByVal NotifyType As String, ByVal TransactionID As Integer) As String
            ...
            With sEmails
                .Append("support@eispro.com;")   'add tech support users
            End With
        ...
        Public Sub FHDAContractAndInsuranceExpirationNotify()
            Dim sql As String
            sql = "SELECT Contacts.Name AS Contractor, InsurancePolicies.PolicyType + ' -- ' + InsurancePolicies.Notes AS PolicyDescription, "
            sql &= "InsurancePolicies.ExpirationDate AS InsuranceExpires FROM Contacts INNER JOIN InsurancePolicies ON Contacts.ContactID = InsurancePolicies.ContactID "
            sql &= "WHERE (Contacts.ContactType = 'Company') AND (Contacts.DistrictID = 55) AND (InsurancePolicies.InsuranceRequired = 1) "
            sql &= "AND (IsDate(InsurancePolicies.ExpirationDate) <> 1) OR "
            sql &= "(Contacts.ContactType = 'Company') AND (Contacts.DistrictID = 55) AND (InsurancePolicies.InsuranceRequired = 1) "
            sql &= "AND (DATEDIFF(day,InsurancePolicies.ExpirationDate, GETDATE()) > - 60)"
