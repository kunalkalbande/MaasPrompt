...
#Region "Projects"
        ...
        Public Sub SaveProject(ByVal CollegeID As Integer, ByVal nProjectID As Integer)
            ...
            SetSchedulePlaceholderEntry(ProjectID)

        End Sub

        Private Sub SetSchedulePlaceholderEntry(ByVal ProjectID As Integer)
            'This Proc creates schecdule placeholder entrye if project has no existing schedule and who have start and end dates set.

            Dim bCreateRow As Boolean = False
            Dim sql As String = "SELECT * FROM ScheduleTasks WHERE ProjectID = " & ProjectID
            db.FillDataTableForUpdate(sql)

            If db.DataTable.Rows.Count < 2 Then   'This has no entries or only placeholder entry
                sql = "SELECT * FROM Projects WHERE ProjectID = " & ProjectID
                Dim tblProject As DataTable = db.ExecuteDataTable(sql)
                Dim projRow As DataRow = tblProject.Rows(0)

                If IsDate(projRow("StartDate")) And IsDate(projRow("EstCompleteDate")) Then


                    If db.DataTable.Rows.Count = 1 Then   'check existing entry
                        Dim row As DataRow = db.DataTable.Rows(0)
                        If row("Name") = "Project Duration (Placeholder)" Then       'Update the dates
                            row("ActualStart") = projRow("StartDate")
                            row("ActualEnd") = projRow("EstCompleteDate")
                            row("Duration") = DateDiff(DateInterval.Day, row("ActualStart"), row("ActualEnd"))
                            row("LastUpdateOn") = Now()
                            row("LastUpdateBy") = HttpContext.Current.Session("UserName")
                    ...
                    Else    'simply add the row as there are none

                        Dim newrow As DataRow = db.DataTable.NewRow()

                        newrow("CollegeID") = projRow("CollegeID")
                        newrow("DistrictID") = projRow("DistrictID")
                        newrow("ProjectID") = projRow("ProjectID")
                        newrow("Name") = "Project Duration (Placeholder)"
                        newrow("Description") = "Auto Generated Schedule placeholder"
                        newrow("ActualStart") = projRow("StartDate")
                        newrow("ActualEnd") = projRow("EstCompleteDate")

                        newrow("PercentComplete") = 0
                        newrow("Parent_TaskID") = 0
                        newrow("Duration") = DateDiff(DateInterval.Day, projRow("StartDate"), projRow("EstCompleteDate"))
        ...
        Public Function GetProjectTotals(ByVal Category As String, ByVal id As Integer) As Double
            ...
            With sql
                Select Case Category
                    ...
                    Case "LedgerAccount"
                        .Append("SELECT SUM(Amount) AS nAmt FROM LedgerAccountEntries WHERE ProjectID = " & id)
